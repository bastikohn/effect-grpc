 ...
 */
export const makeConnect = Effect.fnUntraced(function* (
  options: GrpcConnectInvokerOptions,
): Effect.fn.Return<GrpcInvokerService> {
  ...
  const makeCallSpan = Effect.fnUntraced(function* (
    entry: GrpcMethodEntry,
    spanScope: Scope.Scope,
    onCode: (code: GrpcStatusCode) => void,
  ): Effect.fn.Return<CallSpan> {
