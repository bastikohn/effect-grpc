...
- Handlers observe the incoming deadline through
  ...
  outgoing calls automatically. Use `GrpcDeadline.callOptions` immediately
  before an outgoing RPC for [opt-in budget propagation](server-context.md#propagating-the-remaining-budget).
  Response headers and trailers cannot be set from a handler yet.
...
Everything the generator declares for a well-known type is named under a `Grpc$`
prefix: its converters wherever the type is used (`fromGrpc$GoogleProtobufTimestamp`),
plus the schema and type it stands up when the well-known is a method input or
output and so has no message of its own to name (`Grpc$GoogleProtobufTimestamp`).
`$` cannot appear in a protobuf name, so a message of yours may share the file
with the well-known it is named after.
