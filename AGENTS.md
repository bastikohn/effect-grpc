...
Tests use `@effect/vitest`: `it.effect` (TestClock-backed) for in-process Effect code, `it.live` only where real time or sockets are required, and `assert` rather than `expect`.
